...
class PairedHDF5Dataset(Dataset):
    ...
    def _open_hdf5(self, loc: Tile, map_type: str):
        if map_type == "FOM":
            file_name = "_".join(
                filter(None, [loc.brain, loc.section, loc.region, "FOM_HSV"])
            )
            file_path = os.path.join(self.data_dir, f"{file_name}.h5")
        else:
            # Handle the new transmittance file naming convention
            if loc.region:  # If region exists (left/right)
                file_name = "_".join(
                    filter(None, [loc.brain, loc.section, loc.region, "NTransmittance"])
                )
            else:
                file_name = "_".join(
                    filter(None, [loc.brain, loc.section, "NTransmittance"])
                )
            file_path = os.path.join(self.data_dir, f"{file_name}.h5")
        ...
        if map_type == "FOM" and self._fom_hf is None:
            ...
            if os.path.exists(file_path):
                ...
                raise FileNotFoundError(
                    f"No transmittance HDF5 file found at {file_path}"
                )
    ...
    def __getitem__(self, loc: Tile) -> tuple:
        ...
        fom_patch = fom_image[
            loc.row : loc.row + loc.patch_size, loc.column : loc.column + loc.patch_size
        ]

        # Get transmittance patch
        trans_image = self._trans_hf["pyramid"][resolution_level]
        trans_patch = trans_image[
            loc.row : loc.row + loc.patch_size, loc.column : loc.column + loc.patch_size
        ]
...
class PairedHDF5Sampler(Sampler):
    def __init__(
        self, data_dir: str, brain: str, patch_size: int, tiles_per_epoch: int = 1000
    ) -> None:
        self.data_dir = data_dir
        self.brain = brain
        ...
        self.fom_files = glob.glob(
            os.path.join(self.data_dir, f"{self.brain}*FOM_HSV.h5")
        )
        if not self.fom_files:
            raise FileNotFoundError(f"No FOM files found for brain {self.brain}")

        # Get corresponding transmittance files to verify pairs exist
        self.trans_files = []
        for fom_file in self.fom_files:
            base_name = os.path.basename(fom_file)
            parts = base_name.split("_")
            # Create transmittance file pattern based on whether region exists
            if len(parts) > 3 and parts[2] in ["left", "right"]:
                trans_pattern = f"{parts[0]}_{parts[1]}_{parts[2]}_NTransmittance.h5"
            else:
                trans_pattern = f"{parts[0]}_{parts[1]}_NTransmittance.h5"
            trans_path = os.path.join(self.data_dir, trans_pattern)
            if os.path.exists(trans_path):
                self.trans_files.append(trans_path)
            else:
                print(f"Warning: No matching transmittance file found for {base_name}")
                self.fom_files.remove(fom_file)

        if not self.fom_files:
            raise FileNotFoundError(
                f"No complete FOM-transmittance pairs found for brain {self.brain}"
            )
    ...
    def __iter__(self):
        ...
        for _ in range(self.tiles_per_epoch):
            ...
            tiles.append(
                Tile(self.brain, section, region, "FOM", row, col, self.patch_size)
            )
...
def pad_patch(patch, target_size):
    ...
    if len(patch.shape) == 3:
        ...
        return np.pad(patch, ((0, pad_height), (0, pad_width)), mode="constant")
